...
\item obstajata taki permutaciji $\sigma \in P(n)$ in $\pi \in P(n-1)$, da je 
\begin{equation*}
...
\item Če je $i = 0$, permutacija $\tau$ je cikel, ki vse elemente premakne za eno mesto nazaj, prvi element pa postavi na konec.
\item Če je $i = 0$, permutacija $\tau$ je cikel, ki vse elemente premakne za eno mesto naprej, zadnji element pa postavi na začetek.
\end{enumerate}
Iskana permutacija je potem $\rho = \tau \circ \pi$.
Prepričajmo se, da je $S[i]$ edina izbira za ustreznega soseda. Opazimo, da nam enačba \eqref{eq:orientiran-simpleks} za simpleks $S \in M_k^n$ opiše pot $P$ po točkah $z_0, z_1, \dots, z_n$. S potjo $P$ mislimo na točke $z_0, z_1, \dots, z_n$ in na $1$-dimenzionalna lica simpleksa $S$, ki jih povezujejo. Vsako $1$-dimenzionalno lice iz poti bomo imenovali korak. Koraki so narejeni v smeri vektorjev $\vec{e}_i$ za $i = 1, \dots, n$. Če pogledamo pot samo na licu $L = \langle z_0, \dots, z_{i-1}, z_{i+1}, \dots, z_n \rangle$ imamo dve možnosti.
Pot na licu $L$ je lahko za en korak krajša od poti $P$. V tem primeru lahko pot s korakom podaljšamo na začetku ali na koncu. V enem primeru dobimo simpleks $S$, v drugem pa soseda $S[i]$. To dokaže trditev v primeru, ko je $i \in \{0, n\}$. 
Druga možnost je, da je pot na licu za dva koraka krajša od poti $P$. V Tem primeru imamo dve povezani poti. Povežemo jo lahko na istem mestu, kjer ju poveže pot $P$. V nasprotnem primeru dobimo cikel sestavljen iz baznih vektorjev, kar pa ni mogoče. Torej lahko pot povežemo na dva načina, saj imamo dva različno usmerjena koraka, ki ju lahko uporabimo v dveh različnih vrstnih redih. V enem primeru dobimo simpleks $S$, v drugem pa njegovega soseda $S[i]$. Torej je sosed res enolično določen.
\end{dokaz}
Naj bo $a>0$ realno število in $n$-simpleks $S = [x_0, x_1, \dots, x_n]$, kjer ima za vsak $i = 0, 1, \dots, n$ oglišče $x_i$ edino neničelno koordinato na $i$-tem mestu enako $a$. Triangulacija sompleksa $S$ je simplicialni kompleks, ki vsebuje vse take $n$-simplekse iz mrežne triangulacije $M_\frac{a}{k}^n$ prostora $\R^n$, ki so vsebovani v simpleksu $S$. Naj bo $R = [y_0, y_1, \dots, y_n]$ poljuben $n$-simpleks v prostoru $\R^n$. Definirajmo homeomorfno preslikavo $h : S \to R$ s predpisom
...
S preslikavo $h$ prenesemo triangulacijo simpleksa $S$ na simpleks $R$. Taki triangulaciji rečemo \emph{$k$-ta mrežna delitev} simpleksa $R$. Ker število $a$ ne vpliva na mrežno delitev simpleksa $R$ jo označimo kar $M_k^n$. Radi bi videli, da se v $k$-ti mrežni delitvi s povečevanjem števila $k$ delilni simpleksi zmanjšujejo.
...
Ker so dolžine vseh stranic vsakega simpleksa $R$ iz $k$-te mrežne delitve simpleksa $S$ za faktor $k$ manjše od stranic simpleksa $S$, velja $\diam(R) = \frac{1}{k} \cdot \diam(S)$.
