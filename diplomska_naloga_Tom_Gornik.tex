...
\begin{enumerate}
\item Če je $0 < i < n$,  je $S[i] = \langle z_0, \dots, z_{i-1}, w_i, z_{i+1}, \dots, z_n \rangle$, kjer je $w_i = z_{i-1} + (z_{i+1} - z_i) = z_{i-1} + \vec{e}_{\pi(i)}$.
...
Naj bo $R = <y_0, y_1, \dots, y_n>$ poljuben $n$-simpleks v prostoru $\R^n$. Definirajmo homeomorfno preslikavo $h : S \to R$ s predpisom
...
S preslikavo $h$ smo prenesli triangulacijo simpleksa $S$ na simpleks $R$. Taki triangulaciji rečemo \emph{$k$-ta mrežna delitev} simpleksa $R$. Za oznako $k$-te mrežne delitve $n$-simpleksa bomo uporabljaja kar $M_k^n$, saj število $a$ na to delitev nima vpliva. 
