...
\povzetek{V tem delu je predstavljen elementaren dokaz izreka o invarianci odprtih množic.}
...
\end{dokaz}
Leta -- -- je W. Kulpa dokazal, da je izrek~\ref{izr:PM} ekvivalenten znanemu Brouwerjevemu izreku o negibni točki, ki pravi naslednje.
\begin{izrek}[Brouwerjev izrek o negibni točki]\label{izr:fixedpoint}
Denimo, da imamo kocko $C = [-1, 1]^n$ in zvezno preslikavo $f : C \to C$, potem obstaja točka $x \in C$, da je $f(x) = x$.
\end{izrek}
Izkaže se, da si pri dokazovanju drugih trditev lažje pomagamo z izrekom~\ref{izr:PM}, kot z izrekom~\ref{izr:fixedpoint}. Poglejmo si, kako s pomočjo izreka~\ref{izr:PM} dokažemo izrek o negibni točki. 

\begin{dokaz}[Dokaz izreka \ref{izr:fixedpoint}]
Naj bo $f : C \to C$ poljubna zvezna preslikava iz kocke $C = [-1, 1]^n$ nazaj vase. Definiramo funkcijo $g : C \to \R^n$ s predpisom $g(x) = x - f(x)$. Funkcija $g$ je zvezna in zadošča pogojem izreka~\ref{izr:PM}. Res, za vsak $x \in \I_i^-$ velja, da je $f_i(x) \leq 1$, zato je $g_i(x) = x_i - f_i(x) \leq 1 - 1 =0$. Podobno lahko razmislimo, da je za vsak $x \in \I_i^+$ vrednost funkcije $g_i \geq 0$. Zato obstaja taka točka $z \in C$, da je $g(x) = 0$, kar pomeni, da je $f(x) = x$.
\end{dokaz}
...
\geslo{homeomorfic}{homeomoefen -- prostora $X$ in $Y$ sta homeomorfna , če obstaja homeomorfizem $f : X \to Y$}
